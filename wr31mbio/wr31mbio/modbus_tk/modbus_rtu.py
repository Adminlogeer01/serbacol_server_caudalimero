...
class RtuMaster(Master):
    ...
    def __init__(self, serial, interchar_multiplier=1.5):
        ...
        if self._serial.isOpen():
            print "RtuMaster %s is %s" % (self._serial.portstr, "opened")
        else:
            print "RtuMaster %s is %s" % (self._serial.portstr, "closed")
...
class RtuServer(Server):
    ...
    def __init__(self, serial, databank=None):
        ...
        if databank:
            Server.__init__(self, databank)
        else:
            Server.__init__(self, Databank())
        self._serial = serial
        if self._serial.isOpen():
            LOGGER.info("RtuServer %s is %s" % (self._serial.portstr, "opened"))
        else:
            LOGGER.info("RtuServer %s is %s" % (self._serial.portstr, "closed"))
        self._t0 = utils.calculate_rtu_inter_char(self._serial.baudrate)
