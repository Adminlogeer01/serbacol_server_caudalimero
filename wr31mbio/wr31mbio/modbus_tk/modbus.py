...
class Master:
    ...
    def execute(self, slave, function_code, starting_address, quantity_of_x=0, 
        ...
        if function_code == defines.READ_COILS or function_code == defines.READ_DISCRETE_INPUTS:
            ...
            is_read_function = True
            byte_count = 2 * len(output_value)
            pdu = struct.pack(">BHHHHB", function_code, starting_address, quantity_of_x, starting_address, len(output_value), byte_count)
            for j in output_value:
                if j >= 0:
                    fmt = "H"
                else:
...
def data_locked(fn):
    """Since we're running Python 2.4, we need a decorator to
    replicate the `with threading.Lock` statements in the original
    modbus-tk project"""
...
class Databank:
    ...
    def __init__(self):
        ...
        self._data_lock = threading.RLock() # protect access to the map of slaves
    ...
    def _run_server(self):
        ...
        try:
            ...
            while self._go.isSet():
                self._do_run()
                time.sleep(0.10)
            LOGGER.error("%s has stopped" % self.__class__)
            self._do_exit()
        except Exception, excpt:
